...
import DoughnutChart from '../room1/DoughnutChart';
import BarChart from '../room1/BarChart';
...
const [data, setData] = useState([]);
  ...
  useEffect(() => {
  const fetchData = () => {
    fetchWithRetry('http://raspberrypi.local/api/getDataRoom2.php')
      .then(textData => {
  ...
  const interval = setInterval(fetchData, 2000); // Tự động gọi lại mỗi 5 giây
